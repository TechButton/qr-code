...
<body class="bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-100 p-4 md:p-8">
    <div class="container mx-auto max-w-md bg-white dark:bg-slate-800 p-6 md:p-8 rounded-lg shadow-xl mt-16">
        <h2 class="text-2xl font-bold text-slate-700 dark:text-slate-100 mb-6 text-center">Change Password</h2>
        <?php if ($message): ?>
            <div class="mb-4 p-3 rounded-md bg-<?php echo strpos($message, 'changed') !== false ? 'green' : 'red'; ?>-100 dark:bg-<?php echo strpos($message, 'changed') !== false ? 'green' : 'red'; ?>-900 text-<?php echo strpos($message, 'changed') !== false ? 'green' : 'red'; ?>-700 dark:text-<?php echo strpos($message, 'changed') !== false ? 'green' : 'red'; ?>-200 text-center"><?php echo $message; ?></div>
        <?php endif; ?>
        <form method="post" class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Current Password</label>
                <input type="password" name="current_password" required class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-slate-800 dark:text-slate-100">
            </div>
            <div>
