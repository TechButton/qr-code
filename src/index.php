...
<body class="bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-100 p-4 md:p-8 transition-colors duration-300">
    <div class="container mx-auto max-w-4xl bg-white dark:bg-slate-800 p-6 md:p-8 rounded-lg shadow-xl">
        ...
        <?php if (isset($_SESSION['message'])): ?>
            <div class="mb-6 p-4 rounded-md <?php echo strpos($_SESSION['message_type'], 'success') !== false ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-200'; ?>" role="alert">
                <?php echo $_SESSION['message']; ?>
            </div>
            <?php unset($_SESSION['message']); unset($_SESSION['message_type']); ?>
        <?php endif; ?>

        <section class="mb-10 p-6 bg-slate-50 dark:bg-slate-700 rounded-lg shadow">
            <h2 class="text-2xl font-semibold text-slate-600 dark:text-slate-200 mb-4">Create New QR Code</h2>
            <form action="generate.php" method="POST" class="space-y-4">
                <div>
                    <label for="data_url" class="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">URL or Text to Encode:</label>
                    <input type="text" id="data_url" name="data_url" required
                           class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-slate-800 dark:text-slate-100"
        ...
        <section>
            <h2 class="text-2xl font-semibold text-slate-600 dark:text-slate-200 mb-6">Existing QR Codes</h2>
            <?php if (empty($existing_qr_codes)): ?>
                <p class="text-slate-500 dark:text-slate-400">No QR codes generated yet. Create one above!</p>
            <?php else: ?>
                <div class="space-y-6">
                    <?php foreach ($existing_qr_codes as $qr): ?>
                        <div class="bg-slate-50 dark:bg-slate-700 p-4 rounded-lg shadow-md flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
                            <div class="flex-shrink-0">
                                <img src="<?php echo htmlspecialchars(QR_CODE_DIR . $qr['image_filename']); ?>" alt="QR Code" class="w-32 h-32 md:w-40 md:h-40 border border-slate-200 dark:border-slate-600 rounded-md">
                            </div>
                            <div class="flex-grow text-center md:text-left">
                                <p class="text-sm text-slate-600 dark:text-slate-200 break-all"><strong>Original Data:</strong> <?php echo htmlspecialchars($qr['data_url']); ?></p>
                                ...
                                <p class="text-sm text-slate-500 dark:text-slate-400"><strong>Scans:</strong> <?php echo $qr['scan_count']; ?></p>
                                <p class="text-xs text-slate-400 dark:text-slate-500">Created: <?php echo date("M j, Y, g:i a", strtotime($qr['created_at'])); ?></p>
                            </div>
